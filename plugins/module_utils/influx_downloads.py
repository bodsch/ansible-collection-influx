...
class InfluxDownloads:
    ...
    @staticmethod
    def _parse_v2_release_body(body: str, version: str, arch: str) -> tuple[str, str | None, str]:
        """
        ...
        Returns:
            tuple[str, str | None, str]: (artifact_filename, optional_full_url, sha256)
    ...
    @staticmethod
    def _extract_first_sha256(text: str) -> str | None:
        """Extract the first SHA256 hash from a text blob."""
        m = SHA256_RE.search(text or "")
        return m.group(1) if m else None

    def _fetch(self, url: str, method: str, headers: dict[str, str] | None = None) -> tuple[Any, dict[str, Any]]:
        """
    ...
    def _latest_hint(self, major_version: int | None, edition: str | None) -> str | None:
        """
        Best-effort hint to improve error messages.

        Returns:
            str | None: A short hint string, or None on any failure.
        """
